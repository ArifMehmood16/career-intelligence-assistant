...
| Salary, benefit or logistics line scored as a skill | The model extracts `item_type`; only `requirement` and `responsibility` reach the mapping and the score. Pay, equity, location, travel and right-to-work are `benefit` or `logistics`. Body under a recognized Benefits or Logistics heading, including a Markdown heading, is forced to that kind. A missing or non-enum `item_type`, or a non-boolean `must_have`, is rejected and retried once, then the job is `extraction_incomplete` | A package line that is not under a Benefits or Logistics heading can still be labeled `requirement` until the role is re-analysed |
| Non-work CV text used as evidence | A span must show a responsibility, qualification or outcome before it is a claim or is shown to the assessor. References, hobbies and personal-profile boilerplate are dropped. A repeated span id is rejected. Retrieval returns no candidates when the best claim has no lexical overlap and similarity is below 0.35 | A long line that contains a work verb can still be stored. 0.35 is not a calibrated threshold; PLAN 14.7 owns that |
