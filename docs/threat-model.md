...
| Cover-letter claims treated as experience | Document kind is enforced; uploaded and generated cover letters are excluded from claim extraction, mappings and scoring by hermetic and SQL HTTP regression tests | A user may still copy unsupported claims into a replacement CV; the system can only reason over submitted evidence |
